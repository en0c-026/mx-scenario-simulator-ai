<template>
  <v-sheet v-if="!scenariosFolder" class="d-flex w-100 h-75">
    <v-progress-circular class="ma-auto" size="40" indeterminate></v-progress-circular>
  </v-sheet>
  <v-sheet v-else-if="!scenariosFolder.exists" class="d-flex w-100 h-75">
            ...
            <div v-else class="w-100 ">
              ...
              <v-form class="mb-4">
                ...
                <v-text-field rounded="0" variant="outlined" label="Description"
                  v-model="selectedScenarioFile.content.description"
...
const scenariosFolder = computed(() => {
  ...
  const list = store.files[index].content.map(scenarioFile => {
    try {
  ...
  console.log('list from scenarioFolder', list);
  return {
    exists,
    index,
    list
